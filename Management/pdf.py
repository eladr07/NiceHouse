...
class MultipleDemandWriter:
    ...
    def projectsFlows(self):
        ...
        for d in self.demands:
            ...
            row.append(Paragraph('<br/>'.join([str(i.num) for i in d.invoices.all()])), styleRow9)
            row.append(Paragraph('<br/>'.join([commaise(i.amount) for i in d.invoices.all()])), styleRow9)
            row.append(Paragraph('<br/>'.join([str(p.num) for p in d.payments.all()])), styleRow9)
            row.append(Paragraph('<br/>'.join([commaise(p.amount) for p in d.payments.all()])), styleRow9)
            row.reverse()
            rows.append(row)
