    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return HttpResponseRedirect('/projects/%s/buildings/%s/house/%s' % (project.id, building.id, house.id))
            except:
                pass
        return render_to_response('Management/index.html',
                                  {'locateHouseForm':form},
                                  context_instance=RequestContext(request))
    else:
...
def demand_send(request, id):
    ...
    mail('adush07@gmail.com',#d.project.demand_contact.mail,
         u'עמלה לפרויקט %s לחודש %s' % (d.project, d.month + "/" + d.year),
         '', filename)
