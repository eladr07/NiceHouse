...
class CZilber(models.Model):
    ...
    def calc(self, month):
        ...
        for s in sales:
            if self.base_madad:
                current_madad = Madad.objects.filter(date__year__lte=s.demand.year, 
                                                     date__month__lte=s.demand.month
                                                     ).latest().value
...
class Sale(models.Model):
    ...
    def project_price(self):
        ...
        TAX = Tax.objects.filter(date__lte=date(self.demand.year, self.demand.month,1)).latest().value / 100 + 1
        if c.include_tax:
