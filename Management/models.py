    ...
    def calc(self, month):
        ...
        try:
            ...
            for s in sales:
                ...
                # only calc zilber discount bonus for current demand sales
            ...
            if d.include_zilber_bonus():
                for s in sales:
                    ...
                    logger.debug('sale #%(id)s adds calc values: %(val)s', 
                                 {'id':s.id, 'vals':{'q.count()':q.count(),'old_base':old_base,'sale_add':sale_add}})
