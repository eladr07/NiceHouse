...
class NHCBase(models.Model):
    ...
    def calc(self, nhmonth, ratio = 1):
        ...
        if self.filter.id == NHSaleFilter.His or self.filter.id == NHSaleFilter.All:
            ...
            sales |= NHSaleSide.objects.filter(nhsale__nhmonth = nhmonth, employee2=self.nhemployee)
            sales |= NHSaleSide.objects.filter(nhsale__nhmonth = nhmonth, employee3=self.nhemployee)
            for nhss in sales:
